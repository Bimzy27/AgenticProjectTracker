...
For each project the dashboard SHALL show: display name, current branch, dirty-state indicator (uncommitted changes), count of agent sessions with their states, and latest pipeline status.
...
- **THEN** every registered project is listed with branch, dirty state, session count, and pipeline status
